...
public:
    ...
    void swapFaces(cv::Mat &frame_ann, cv::Rect &rect_ann, cv::Mat &frame_bob, cv::Rect &rect_bob);

private:
    // Returns minimal Mat containing both faces
    void setMinFrames(const cv::Mat &frame_ann, cv::Rect &rect_ann, const cv::Mat &frame_bob, cv::Rect &rect_bob);

    cv::Rect getBoudingRect(const cv::Mat &frame, cv::Rect rect);

    cv::Rect getBigRect(const cv::Rect& rect, const cv::Rect& boundingRect);

    // Finds facial landmarks on faces and extracts the useful points
    void getFacePoints(const cv::Mat &frame_ann, const cv::Mat &frame_bob);
    ...
    cv::Mat small_frame_ann;
    cv::Mat small_frame_bob;
