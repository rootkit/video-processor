...
void FaceSwapper::swapFaces(cv::Mat &frame_ann, cv::Rect &rect_ann, cv::Mat &frame_bob, cv::Rect &rect_bob)
{
    setMinFrames(frame_ann, rect_ann, frame_bob, rect_bob);

    getFacePoints(small_frame_ann, small_frame_bob);
...
void FaceSwapper::setMinFrames(const cv::Mat &frame_ann, cv::Rect &rect_ann, const cv::Mat &frame_bob, cv::Rect &rect_bob)
{
    auto boundingRectAnn = getBoudingRect(frame_ann, rect_ann);
    auto boundingRectBob = getBoudingRect(frame_bob, rect_bob);

    auto width = cv::max(boundingRectAnn.width, boundingRectBob.width);
    auto height = cv::max(boundingRectAnn.height, boundingRectBob.height);

    boundingRectAnn.width = boundingRectBob.width = width;
    boundingRectAnn.height = boundingRectBob.height = height;

    boundingRectAnn &= cv::Rect(0, 0, frame_ann.cols, frame_ann.rows);
    boundingRectBob &= cv::Rect(0, 0, frame_bob.cols, frame_bob.rows);

    width = cv::min(boundingRectAnn.width, boundingRectBob.width);
    height = cv::min(boundingRectAnn.height, boundingRectBob.height);

    boundingRectAnn.width = boundingRectBob.width = width;
    boundingRectAnn.height = boundingRectBob.height = height;

    this->rect_ann = rect_ann - boundingRectAnn.tl();
    this->rect_bob = rect_bob - boundingRectBob.tl();

    small_frame_ann = frame_ann(boundingRectAnn);
    small_frame_bob = frame_bob(boundingRectBob);

    this->big_rect_ann = getBigRect(this->rect_ann, boundingRectAnn);
    this->big_rect_bob = getBigRect(this->rect_bob, boundingRectBob);

    frame_size = cv::Size(small_frame_ann.cols, small_frame_ann.rows);
}

cv::Rect FaceSwapper::getBigRect(const cv::Rect& rect, const cv::Rect& boundingRect)
{
    return ((rect - cv::Point(rect.width / 4, rect.height / 4)) + cv::Size(rect.width / 2, rect.height / 2)) & cv::Rect(0, 0, boundingRect.width, boundingRect.height);
}

cv::Rect FaceSwapper::getBoudingRect(const cv::Mat &frame, cv::Rect rect)
{
    cv::Rect bounding_rect = rect;
...
void FaceSwapper::getFacePoints(const cv::Mat &frame_ann, const cv::Mat &frame_bob)
{
    ...
    dlib::cv_image<dlib::bgr_pixel> dlib_frame_ann = frame_ann;
    dlib::cv_image<dlib::bgr_pixel> dlib_frame_bob = frame_bob;

    shapes[0] = pose_model(dlib_frame_ann, dlib_rects[0]);
    shapes[1] = pose_model(dlib_frame_bob, dlib_rects[1]);
...
{
    small_frame_ann.copyTo(face_ann, mask_ann);
    small_frame_bob.copyTo(face_bob, mask_bob);
}
...
{
    specifiyHistogram(small_frame_ann(big_rect_ann), warpped_faces(big_rect_ann), warpped_mask_bob(big_rect_ann));
}
...
{
    for (size_t i = 0; i < small_frame_ann.rows; i++) {
        auto frame_pixel = small_frame_ann.row(i).data;
        auto faces_pixel = warpped_faces.row(i).data;
        auto masks_pixel = refined_masks.row(i).data;

        for (size_t j = 0; j < small_frame_ann.cols; j++) {
            if (*masks_pixel != 0) {
