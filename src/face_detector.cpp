...
FaceDetectorAndTracker::FaceDetectorAndTracker(const std::string cascadeFilePath, size_t numFaces)
{
...
void FaceDetectorAndTracker::setInput(cv::VideoCapture* input)
{
    videoInput = input;
    ...
    cv::Size size((int)videoInput->get(cv::CAP_PROP_FRAME_WIDTH),
...
cv::Size FaceDetectorAndTracker::originalFrameSize()
{
    return m_originalFrameSize;
}
...
void FaceDetectorAndTracker::processFrame(const cv::Mat& frame)
{
...
void FaceDetectorAndTracker::detect()
{
    ...
    m_faceCascade->detectMultiScale(m_downscaledFrame, m_facesRects, 1.1, 3, 0,
        cv::Size(m_downscaledFrame.rows / 10, m_downscaledFrame.rows / 10),
